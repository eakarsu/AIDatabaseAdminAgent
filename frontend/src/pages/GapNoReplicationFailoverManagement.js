...
/**
 * Gap/CFS Feature Page: No replication/failover management.
 * Slug: no-replication-failover-management
 * Backend: /api/gap-no-replication-failover-management
 */
export default function NoReplicationFailoverManagementPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-replication-failover-management', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No replication/failover management.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No replication/failover management.</p>
