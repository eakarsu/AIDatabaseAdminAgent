...
/**
 * Gap/CFS Feature Page: Limited cloud DB integrations — no AWS RDS / Azure SQL / GCP Cloud SQL adapter.
 * Slug: limited-cloud-db-integrations-no-aws-rds-azure-sql-gcp-cloud
 * Backend: /api/gap-limited-cloud-db-integrations-no-aws-rds-azure-sql-gcp-cloud
 */
export default function LimitedCloudDbIntegrationsNoAwsRdsAzureSqlGcpCloudPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-cloud-db-integrations-no-aws-rds-azure-sql-gcp-cloud', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Limited cloud DB integrations — no AWS RDS / Azure SQL / GCP Cloud SQL adapter.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Limited cloud DB integrations — no AWS RDS / Azure SQL / GCP Cloud SQL adapter.</p>
