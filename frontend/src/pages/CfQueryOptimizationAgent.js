...
/**
 * Gap/CFS Feature Page: Query optimization agent
 * Slug: query-optimization-agent
 * Backend: /api/cf-query-optimization-agent
 */
export default function QueryOptimizationAgentPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-query-optimization-agent', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Query optimization agent</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Analyze slow queries; recommend indexes, schema changes, query rewrites; auto-apply optimizations.</p>
