-- Additive migration: tenant-scoped, approval-gated database_change workflow.
...
CREATE TABLE IF NOT EXISTS governed_cases (
  ...
  case_kind VARCHAR(64) NOT NULL CHECK (case_kind = 'database_change'),
  business_key VARCHAR(160) NOT NULL,
  state VARCHAR(64) NOT NULL CHECK (state IN ('proposed', 'evidence_collected', 'sandbox_ready', 'sandbox_passed', 'approval_pending', 'approved', 'executing', 'verified', 'rolled_back', 'failed', 'closed')),
...
CREATE TABLE IF NOT EXISTS governed_integration_runs (
  id BIGSERIAL PRIMARY KEY,
  tenant_id VARCHAR(128) NOT NULL,
  connector VARCHAR(64) NOT NULL CHECK (connector IN ('database_driver', 'metrics_tracing', 'secret_vault', 'migration_tool', 'ticketing', 'backup_system')),
...
CREATE TABLE IF NOT EXISTS governed_quality_observations (
  ...
  tenant_id VARCHAR(128) NOT NULL,
  case_id BIGINT REFERENCES governed_cases(id) ON DELETE RESTRICT,
  metric_name VARCHAR(80) NOT NULL CHECK (metric_name IN ('lock_impact', 'load_delta', 'sandbox_success', 'rollback_success', 'recovery_time', 'recommendation_accuracy')),
