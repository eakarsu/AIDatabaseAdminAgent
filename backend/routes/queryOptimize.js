...
const FEATURE_KEY = process.env.FEATURE_QUERY_OPTIMIZE_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Query optimization agent.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn(`${baseUrl}/chat/completions`, {
    ...
    headers: {
      ...
      'X-Title': 'AIDatabaseAdminAgent - query-optimize',
    },
    body: JSON.stringify({
...
router.post('/query-optimize', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_QUERY_OPTIMIZE_KEY');
    const ai = await callLLM({ feature: 'query-optimize', goal: 'Query optimization agent', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['query-optimize', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[query-optimize] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'query-optimize',
      endpoint: '/api/ai/query-optimize',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[query-optimize] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/query-optimize/health', (req, res) => {
  res.json({
    feature: 'query-optimize',
    endpoint: '/api/ai/query-optimize',
