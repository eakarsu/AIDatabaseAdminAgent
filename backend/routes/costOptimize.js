...
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Cloud DB cost optimization.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDatabaseAdminAgent - cost-optimize',
    },
    body: JSON.stringify({
...
router.post('/cost-optimize', auth, async (req, res) => {
  try {
    ...
    const ai = await callLLM({ feature: 'cost-optimize', goal: 'Cloud DB cost optimization', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
